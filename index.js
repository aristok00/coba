...
/////////////////
const BotName = 'AristokBots'; 
const instagram = 'www.instagram.com/aris_tokk'; 
const kapanbotaktif = 'Sampai Ga Aktiff'; 
const
{
   WAConnection,
   MessageType,
...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] Scan kode qr dengan whatsapp gengs!`);
});
...
conn.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log('Bot by ig:@aris_tokk')
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by ig:@aris_tokk`)
})
...
{
  conn.sendMessage(id, 'Ada yang bisa di bantu dengan AristokBot?', MessageType.text);
}
else if (text == 'ris')
{
  conn.sendMessage(id, 'Ada yang bisa di bantu dengan AristokBot?', MessageType.text);
...
{
  conn.sendMessage(id, 'Opo cokk, -AristokBot', MessageType.text);
}
else if (text == 'cok')
{
  conn.sendMessage(id, 'Opo cokk, -AristokBot', MessageType.text);
...
{
  conn.sendMessage(id, 'Opo mbot, -AristokBot?', MessageType.text);
}
else if (text == 'mbot')
{
  conn.sendMessage(id, 'Opo mbot, -AristokBot', MessageType.text);
...
{
  conn.sendMessage(id, 'Sek tak mikir aku ng ndi', MessageType.text);
}
else if (text == 'posisi')
{
  conn.sendMessage(id, 'Sek tak mikir aku ng ndi', MessageType.text);
...
axios.get(`https://st4rz.herokuapp.com/api/yta?url=${teks}`).then((res) => {
    let hasil = `Audio telah tersedia pada link di bawah, download dewe ojok manja hasilnya\n👇👇👇👇👇👇👇👇👇\n\nJudul: ${res.data.title}\n\nUkuran audio: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://st4rz.herokuapp.com/api/ytv?url=${teks}`).then((res) => {
    let hasil = `Video telah tersedia pada link di bawah, download dewe ojok manja hasilnya\n👇👇👇👇👇👇👇👇👇\n\nJudul: ${res.data.title}\n\nUkuran video: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/epbe?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
    let hasil = `Berhasil! download dewe ojok manja!\n👇👇👇👇👇👇👇👇👇\n\nJudul: ${res.data.title}\n\nUkuran: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
conn.sendMessage(id, menu.menu(id, BotName, tampilTanggal, tampilWaktu, instagram, kapanbotaktif) ,MessageType.text);
}
else if (text == '!quran'){
...
var tampilTanggal = "TANGGAL: " + hari + ", " + tanggal + " " + bulan + " " + tahun;
var tampilWaktu = "JAM: " + jam + ":" + menit + ":" + detik;
conn.sendMessage(id, info.info(id, BotName, tampilTanggal, tampilWaktu, instagram, kapanbotaktif) ,MessageType.text);
}
else if (text == '!pict'){
...
await new Promise(resolve => setTimeout(resolve, 500));
	...
	if(suara.length > 200){ // check longness of text, because otherways google translate will give me a empty file
  conn.sendMessage("Tulisanmu kedawan suu!")
}else{
